...
test_that("Blob and ADLS mixed updating works on blob container in non-HNS-enabled account",
{
    ...
    expect_true(file.exists(file.path(destdir, "adlsfoo/bar/iris.csv")))
    expect_true(file.exists(file.path(destdir, "blobfoo/bar/iris.csv")))
...
test_that("Blob and ADLS mixed updating works on ADLS filesystem in non-HNS-enabled account",
{
    cont_name <- paste0(sample(letters, 10, TRUE), collapse="")
