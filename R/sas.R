...
#' @rdname sas
#' @export
get_service_sas.az_storage <- function(account, resource, service_type=c("blob", "file"), key=account$list_keys()[1],
                                       ...)
{
    service_type <- match.arg(service_type)
    endp <- if(service_type == "blob")
...
get_service_sas.storage_endpoint <- function(account, resource, key=account$key, ...)
{
    ...
    service_type <- sub("_endpoint$", "", class(account)[1])
    acctname <- sub("\\..*", "", httr::parse_url(account$url)$hostname)
    get_service_sas(acctname, resource=resource, key=key, service_type=service_type, ...)
}


#' @param service_type For a service SAS, the storage service for which the SAS is valid: either "blob" or "file". Currently AzureStor does not support creating a service SAS for queue or table storage.
...
#' @param snapshot_time For a service SAS, the specific blob snapshot for which the SAS is valid, if `resource_type` is "bs".
#' @rdname sas
#' @export
get_service_sas.default <- function(account, resource, key, service_type, resource_type=NULL, start=NULL, expiry=NULL,
                                    permissions="rl", ip=NULL, protocol=NULL, policy=NULL, snapshot_time=NULL,
                                    auth_api_version=getOption("azure_storage_api_version"), ...)
{
    if(!(service_type %in% c("blob", "file")))
        stop("Generating a service SAS currently only supported for blob and file storage", call.=FALSE)

    dates <- make_sas_dates(start, expiry)
    if(is.null(resource_type))
        resource_type <- if(service_type == "blob") "b" else "f"
    resource <- file.path("", service_type, account, resource)  # canonicalized resource starts with /
